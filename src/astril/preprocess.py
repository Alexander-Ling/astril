...
def match_direction_matrices(input_path, donor_path, output_path, *, debug: bool = False):
    ...
    * Setting qform can fail for rank-deficient affines (cannot be decomposed into a quaternion).
    ...
    try:
        ...
        from .preprocessing_utils import preflight_sitk_read_in_subprocess

        preflight_sitk_read_in_subprocess(donor_path, full_read=True, timeout_sec=120)
        preflight_sitk_read_in_subprocess(input_path, full_read=True, timeout_sec=120)
...
def register_images(
    ...
    **3D and 4D support**
    ...
    def _read_image_any(path_like) -> sitk.Image:
        ...
        preflight_sitk_read_in_subprocess(p, full_read=True, timeout_sec=120)
...
def inverse_transform_image(
    ...
    *,
    ...
    import SimpleITK as sitk
    from .preprocessing_utils import (
        sitk_extract_3d_from_4d,
        sitk_join_3d_frames_to_4d,
        preflight_sitk_read_in_subprocess,
    )
    ...
    def _get_image_ndim(path):
        import SimpleITK as sitk
        try:
            preflight_sitk_read_in_subprocess(path, full_read=False, timeout_sec=60)
            r = sitk.ImageFileReader()
            r.SetFileName(str(path))
            r.ReadImageInformation()
        except Exception as e:
            raise RuntimeError(f"Failed to load image metadata: {path}\n{e}")
        return int(r.GetDimension()), tuple(int(v) for v in r.GetSize())
...
def inverse_transform_image(
    ...
    *,
    ...
    preflight_sitk_read_in_subprocess(original_image_path, full_read=True, timeout_sec=120)
    preflight_sitk_read_in_subprocess(transformed_image_path, full_read=True, timeout_sec=120)
